...
import filtering, evaluation, utils  # onlineldavb, os, parallelfiltering, onlinevb, asynchronous
...
def parse_args():
    ...
    parser.set_defaults(K=100, D=-1, corpus = "nature", alpha = 1.0, eta = 1.0, batchsize=1024,
        async_batches_per_eval=5, max_iters=100, threshold=1.0, numthreads=1, tau0=1.0, kappa=0.9,
        useHBBBound = True, minNumPtsPerEval = -1, expGrowthEval=1)
    ...
    parser.add_option("--datadir", type="string", dest="datadir", default="./",
                      help="Directory of data files (default=./)")
    parser.add_option("--numinit", type="int", dest="numinit", default=10,
                      help="Number of initial labels (default=10)")
...
def main():
    ...
    wikirandom = archived_dataset.Corpus(datadir + corpus + "_train.dat") # should be _train.dat
    ...
    try:
        ...
        print("No model file found.  Loading defaults.")
        eta = options.eta  # prior for topics
        K = options.K      # number of topics
        alpha = 1./K
        Nl = K
        ppi = 1./K * np.ones((K,Nl))

    # load all labels
    labels_all = archived_dataset.loadLabels(datadir + corpus + "_labels.dat")
    ...
    if (algorithmname == "filtering_ep"):
        if (numthreads == 1):
            alg = filtering.FilteringEP(\
                    W, K, alpha, eta, options.max_iters, options.threshold, options.useNewton)
        else:
            if (options.async):
                alg = asynchronous.ParallelFilteringEP(\
                        W, K, alpha, eta, options.max_iters, options.threshold, \
                        options.useNewton, options.batchsize, options.numthreads)
                batchsize = batchsize * options.async_batches_per_eval * options.numthreads
            else:
                alg = parallelfiltering.ParallelFilteringEP(W, K, alpha, eta, \
                        options.max_iters, options.threshold, options.useNewton, \
                        options.numthreads, options.batchsize)
                batchsize = batchsize * options.numthreads

    # EP for Labeled LDA
    if (algorithmname == "llda_filtering_ep"):
        if (numthreads == 1):
            alg = filtering.LLDAFilteringEP(\
                    W, K, alpha, eta, ppi, model.Nl, labels, options.max_iters, \
                    options.threshold, options.useNewton, silent=False)
        else:
            raise Exception("Parallel EP for LLDA not implemented.")


    # Specify the minimum number of points to be processed before we run the evaluation code,
    # since evaluation is expensive
    ...
    print("Using algorithm: " + str(algorithmname))
    recordedData = []
    totalTime = 0.0
    ...
    for iteration in range(iters):
        ...
        # evaluate if we have processed enough points, or this is the last iteration
        if (numPtsProc >= minNumPtsPerEval or iteration == iters-1):
            numPtsProc = 0  # reset the counter

            # The following is just the usual evaluation code from before
            start = time.time()
            (perplex, split) = evaluation.evaluate(\
                validation_docs, alg_alpha, alg_lam, options.usePtEst)
            testTime = time.time() - start
            print(str(iteration+1) + "/" + str(iters) + " " + str(algorithmname) +
                  " (%g, %g): held-out perplexity estimate = %f, %f"
                  % (iter_time, testTime, perplex, split))

            # also save perplexity now
            recordedData += [((iteration+1)*batchsize, totalTime, totalDownloadingTime, \
                              perplex, split)]
            if (algorithmname in ["hbb", "filtering", "filtering_ep", "filtering_ep2"]):
                # need to distinguish eta now
    	        outfile = corpus + "_" + algorithmname + "_" + str(batchsize) + "_eta" + str(eta)
            else:
    	        outfile = corpus + "_" + algorithmname + "_" + str(options.batchsize) + \
                          "_" + str(options.numthreads) + "_eta" + str(eta)
            numpy.save(datadir + outfile, recordedData)
        ...
        if (iteration == iters-1):
            ...
            if (algorithmname in ["hbb", "filtering", "filtering_ep", "filtering_ep2"]):
                # need to distinguish eta now
                topics_outfile = "topics_" + corpus + "_" + algorithmname \
                                 + "_" + str(batchsize) + "_eta" + str(eta)
            else:
                topics_outfile = "topics_" + corpus + "_" + algorithmname + "_" \
                                 + str(options.batchsize) + "_" + str(options.numthreads)
            numpy.save(datadir + topics_outfile, alg_lam)
            print("Saved to: " + topics_outfile)
