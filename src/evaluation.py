...
def lda_e_step(words_counts, alpha, k, Elogbeta, max_iter=100):
    (words, counts) = words_counts
    gamma = np.ones(k)  
...
def lda_e_step_full(words_counts, alpha, k, lam, max_iter=100):
    (words, counts) = words_counts
    gamma = np.ones(k)  
    expElogtheta = np.exp(dirichlet_expectation(gamma)) 
...
def lda_e_step_split(words_counts, alpha, k, beta, max_iter=100):
    (words, counts) = words_counts
    length = len(words)
...
def lda_e_step_split_full(words_counts, alpha, k, lam, max_iter=100):
    (words, counts) = words_counts
    length = len(words)
    half_len = int(len(words) / 2) + 1
    idx_train = [2*i for i in range(half_len) if 2*i < length]
