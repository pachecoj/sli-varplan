...
import pickle, string, numpy, getopt, sys, random, time, re, pprint, copy, math
import ep_llda # onlineldavb, batchvb, ep2_lda
...
def runBatchEPLLDA(map_args):
        (W, K, Nl, labels, docs, alpha, eta, maxiters, threshold, ppi, useNewton, silent) = map_args
        ep = ep_llda.EP_LLDA(W, K, Nl, labels, docs, alpha, eta, ppi, useNewton, silent)
        lam = ep.train(maxiters, threshold)
        return lam

# def runBatchVB(xxx_todo_changeme):
#         (W, K, docs, alpha, lam, maxiters,thr,hbb) = xxx_todo_changeme
#         batchVB = batchvb.BatchLDA(W, K, docs, alpha, lam, useHBBBound = hbb)
#         lam = batchVB.train(maxiters, thr)
#         return lam

# # For ParallelFilteringEP
# def runBatchEP(xxx_todo_changeme1):
#         (W, K, docs, alpha, lam, maxiters, threshold, useNewton) = xxx_todo_changeme1
#         ep = ep_lda.EP_LDA(W, K, docs, alpha, lam, useNewton)
#         lam = ep.train(maxiters, threshold)
#         return lam

# # For ParallelFilteringEP2
# def runBatchEP2(xxx_todo_changeme2):
#         (W, K, docs, alpha, lam, maxiters, threshold, useNewton) = xxx_todo_changeme2
#         ep2 = ep2_lda.EP2_LDA(W, K, docs, alpha, lam, useNewton)
#         lam = ep2.train(maxiters, threshold)
#         return lam


# Labeled LDA Parallel filtering with EP
class LLDAParallelFilteringEP:
    def __init__(self, W, K, alpha, eta, ppi, Nl, labels, maxiters, \
                threshold, useNewton, numChunks, batchsize, silent=False):
        """
        Arguments:
        K: Number of topics
        ...
        numChunks: Num. of partitions of data to distribute to workers
        """
        self._str = "ParallelFilteringEP_%d_%g_%r_%dx_%d" % \
                    (maxiters,threshold,useNewton,numChunks,batchsize)
    ...
    def update_lambda(self,docs):
        ...
        estimates = self._pool.map(runBatchEPLLDA,
           [ (self._W, self._K, self._Nl, self._labels, docs, self._alpha, \
              self._lambda, self._maxiters, self._threshold, self._ppi, \
              self._useNewton, self._silent) for docs in chunks])
