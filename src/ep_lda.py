...
class EP_LDA:
    ...
    def __init__(self, W, K, docs, alpha, eta, useNewton, silent=False):
        """
        Arguments:
        K: Number of topics
        ...
        for k in range(0, self._K):
            omega_k = list()
            for d in range(0, self._D):
                omega_k.append(np.random.rand(self._N[d], self._W)) # np.zeros([self._N[d], self._W]))
        ...
        self._lambda = self._eta + np.sum(np.sum(self._omega, axis=2), axis=1)
    ...
    def train(self, maxiters, thr, silent = False):
        ...
        for iteration in range(0,maxiters):
            ...
            # ## DEBUG: Display topics
            # if iteration < 5:
            #     (fig, axs) = show_bars_topics(self._lambda, self._W, self._K)
            #     fig.suptitle('Iter: %d' % iteration)

            # Compute new gamma, and check for convergence
            delta = self.compute_diff(old_lambda, old_gamma)
            if not self._silent:
                print( "\tIter " + str(iteration+1) +"/" + str(maxiters) + \
                       ": delta = " + str(delta) + " (thr = " + str(thr) + ")") #not pythonic

            # check convergence
            if delta < thr:
                if not self._silent:
                    print("\tBatchEP converged in " + str(iteration+1) + \
                          " iterations, delta = " + str(delta) + " (thr = " + str(thr) \
    ...
    def update_omega_zeta(self):
        ...
        """
        for d in range(0, self._D):
            # print("\tAt doc " + str(d+1) + "/" + str(self._D) + \
            #       ": N_d =  " + str(self._N[d]) )
            for n in range(0, self._N[d]):
                # print("\t\td = " + str(d+1) + "/" + str(self._D) + \
                #       ", n = " + str(n+1) + "/" + str(self._N[d]))
                ...
                # this is an integer between 0 and W-1 (size of the vocabulary)
                ...
                E_theta_d2 = \
                    (rho_d * (rho_d + 1) + 2 * C * (rho_d + 1)) / ((sum_rho_d + 1) * (sum_rho_d + 2))
