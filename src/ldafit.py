...
import filtering, evaluation # onlineldavb, os, utils, parallelfiltering, onlinevb, asynchronous
        ...
        if (numthreads == 1):
            ...
            if (options.async):
                alg = asynchronous.ParallelFilteringEP(\
                        W, K, alpha, eta, options.max_iters, options.threshold, options.useNewton, options.batchsize, options.numthreads)
                batchsize = batchsize * options.async_batches_per_eval * options.numthreads
            else:
    ...
    for iteration in range(iters):
        ...
        if (numPtsProc >= minNumPtsPerEval or iteration == iters-1):  
            ...
            print(str(iteration+1) + "/" + str(iters) + " " + str(algorithmname) +
                  " (%g, %g): held-out perplexity estimate = %f, %f" % (iter_time, testTime, perplex, split))

            # also save perplexity now
            recordedData += [((iteration+1)*batchsize, totalTime, totalDownloadingTime, perplex, split)]  
